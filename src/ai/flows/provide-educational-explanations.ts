// src/ai/flows/provide-educational-explanations.ts
'use server';
/**
...
const EducationalExplanationsInputSchema = z.object({
  topic: z.string().describe('The topic for which an explanation is needed (e.g., product name, technique, routine step).'),
  context: z.string().optional().describe('Additional context or user question related to the topic.'),
});
export type EducationalExplanationsInput = z.infer<typeof EducationalExplanationsInputSchema>;

const EducationalExplanationsOutputSchema = z.object({
  explanation: z.string().describe('A detailed explanation of the topic, including ingredient information, brand background, technique definitions, and tutorial suggestions.'),
});
export type EducationalExplanationsOutput = z.infer<typeof EducationalExplanationsOutputSchema>;

export async function provideEducationalExplanations(input: EducationalExplanationsInput): Promise<EducationalExplanationsOutput> {
